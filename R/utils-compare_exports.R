#' @export
compare_exports <- function(pkg, current = NULL, wait = 2) {
  if (is.null(current)) {
    wd <- getwd()
  ...
  if (inherits(sleep, c("error", "warning"))) {
    cat("\nWarning or Error occurred. Retying in", wait, "seconds.\n")
    Sys.sleep(wait)
  ...
  xprts <- Filter(
    Negate(is.null),
    mget(getNamespaceExports(ns = ns),
      envir = ns,
      mode = "function",
