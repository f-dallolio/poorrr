...
str_split <- function(string, pattern, n = Inf, simplify = FALSE) {
  if (simplify) {
    ...
    str_split_raw(string, pattern, n)
  }
}
#' @rdname utils-string
#' @export
str_split_fixed <- function(string, pattern, n = Inf) {
  x <- str_split_raw(string, pattern, n)
  nn <- lengths(x)
  if (is.infinite(n)) {
    n <- max(nn)
  }
...
str_split_raw <- function(string, pattern, n = Inf) {
  if (length(string) > 1) {
    return(lapply(string, str_split_raw, pattern = pattern, n = n))
  }
  x <- strsplit(string, pattern)[[1]]
  out <- head(x, n)
  if (is.infinite(n) || n >= length(x)) {
    return(out)
  }
  out[n] <- paste(c(out[n], setdiff(x, out)), collapse = pattern)
  out
}
