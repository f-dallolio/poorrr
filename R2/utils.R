as_is_node <- function(f, error_call = caller_env(), error_arg = caller_arg(f)) {
  if (is.null(f)) {
    ...
    as_predicate(is_node_f,
      .mapper = FALSE, .purrr_error_call = error_call,
...
as_closure <- function(fn) {
  ...
  if (is.null(out)) {
    ...
    out <- as.function(c(fmls, call("{", as.call(c(
      as.symbol(name),
      unname(syms)
    )))), envir = topenv())
    return(out)
...
list_names <- function(x, enframe = FALSE, unnest = FALSE) {
  ...
  if (unnest) {
    out <- data_frame(outer = rep(nms, lengths(out)), inner = do.call(
      "c",
      unname(out)
    ))
...
is_deprecated <- function(fn) {
  ...
  out_name <- call_name(x) %in% c(
    "signal_stage", "deprecate_warn",
    "deprecate_stop", "deprecate_soft"
  )
...
is_superseded <- function(fn) {
  ...
  out_name <- call_name(x) %in% c("signal_superseded", "signal_stage")
