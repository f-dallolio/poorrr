...
#' @export
list_simplify <- function(x, ..., strict = TRUE, error_arg = caller_arg(x), error_call = caller_env()) {
  if (strict) {
...
list_simplify0 <- function(x, ..., error_arg = caller_arg(x), error_call = caller_env()) {
  if (!obj_is_list(x) || !list_all_vectors(x) || !list_all_size(
    x,
    1
  )) {
    return(x)
