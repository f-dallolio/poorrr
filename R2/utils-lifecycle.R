pkg_objs <- function(pkg) {
  ns <- asNamespace(pkg)
  objs <- unlist(mget(names(ns), ns,
    ifnotfound = list(NULL),
    inherits = FALSE
  ))
  x <- data.frame(
    obj = names(objs), export = names(objs) %in%
      getNamespaceExports(pkg), mode = map_chr(objs, mode),
    type = map_chr(objs, mode), class = map_chr(objs, function(x) class(x)[[1]])
  )
...
pkg_lifecycle_statuses <- function(package, which = c(
           "superseded", "deprecated", "questioning",
           "defunct", "experimental", "soft-deprecated", "retired"
         )) {
  ...
  lc <- db_lifecycle(db)
  funs <- db_function(db)
  res <- mapply(function(lc, f) {
    data.frame(fun = f, lifecycle = rep(
      lc,
      length(f)
    ), stringsAsFactors = FALSE)
  }, lc, funs, SIMPLIFY = FALSE)
...
db_lifecycle <- function(db) {
  lifecycle_patterns <- paste0(
    "(?:", paste(
      collapse = "|",
      c(
        "lifecycle::badge\\([\\\\]\"", "rlang:::lifecycle\\([\\\\]\"",
...
db_function <- function(db) {
  usage <- lapply(
    db, asNamespace("tools")$.Rd_get_section,
    "usage"
  )
